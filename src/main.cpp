...
uint256 hashGenesisBlock("0x3e9a7d892ad3470dc816332d9be0ec699a1850193d2dbfb9822e3044a792a667");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // 1CRedit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "1CRedit Signed Message:\n";
...
{
    // 1CRedit: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
	    ...
	    printf("txout.nValue=%lld, MAX_MONEY=%lld\n", txout.nValue, MAX_MONEY);
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (nBlockSize != 1 && nNewBlockSize >= MAX_BLOCK_SIZE/4)
    {
        if (nNewBlockSize >= MAX_BLOCK_SIZE)
            return MAX_MONEY;
        nMinFee *= MAX_BLOCK_SIZE / (MAX_BLOCK_SIZE - nNewBlockSize);
    }
...
// Modified for 1Credit - check every block using average of last hour
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
   ...
   printf("nActualTargetTime = %"PRI64d"   nActualTimespan = %"PRI64d"  over %d blocks\n", nActualTargetTime, nActualTimespan,nLookBackCount);
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // 1CRedit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // 1CRedit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397936191;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086399964;

        if (fTestNet)
        {
            block.nTime    = 1397936191;
            block.nNonce   = 386258989;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x73748f8975cf353e55eed54ceeff72096ce3319a0fae26893ce83a62972ecd8f"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc6, 0xbf, 0xe1 }; // 1CRedit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("1CReditMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("1CReditMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OneCReditMiner(CWallet *pwallet)
{
    printf("1CReditMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("1credit-miner");
    ...
    try { while (true)  {
        ...
        printf("Running 1CReditMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("1CReditMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OneCReditMiner, pwallet));
}
